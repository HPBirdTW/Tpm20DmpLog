...
)
{
	...
	UINTN					EventTblSize;
	EFI_STATUS(*HashFunc)(UINT8 * Buf, UINTN BufLen, UINT8 * mac) = NULL;
...
)
{
	...
	Status = TpmSubmitCommand(SendBufferSize, (UINT8*)&SendBuffer, &RecvBufferSize, (UINT8*)&RecvBuffer);
	...
	{
		Status = SwapBytes32(RecvBuffer.Header.responseCode) | MAX_BIT;
		return Status;
	}
...
)
{
	...
	do
	{
		...
		Status = TpmSubmitCommand(sizeof(Tpm2CapAlgs), Tpm2CapAlgs, &RecvBufferSize, TpmRespBuf);
		if (EFI_ERROR(Status))
		{
			break;
		}

		CopyMem(&u32TmpVal, &TpmRespBuf[0x0F], sizeof(u32TmpVal));
		...
		{
			CopyMem(&AlgID, pByte, sizeof(UINT16));
			pByte += 2;
			AlgID = SwapBytes16(AlgID);
			CopyMem(&u32TmpVal, pByte, sizeof(u32TmpVal));
			pByte += 4;
